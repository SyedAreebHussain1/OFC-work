...
const PaymentPlan = (props) => {
  ...
  useEffect(() => {
    if (props.state.noOfMonths !== "") {
      let DiscountAmount = props.state.afterDiscount;
      let DownPayment = props.state.downpaymentAmount;

      let possession = props.state.possessionAmount;
      let eachinstallmentvalue = props.state.installmentAmount;

      let eachBaloonInstallmentAmount = props.state.perBalloonInst;

      let TotalNoOfInstallments =
        props.state.noOfMonthlyInst > 0
          ? props.state.noOfMonthlyInst +
          (props.state.noOfBalloonInst > 0 ? props.state.noOfBalloonInst : 0)
          : props.state?.paymentPlan;
      let noOfBallon = props.state.noOfBalloonInst;

      let netAmount = props.state.netAmount;
      let remainingAmount = props.state.netAmount;
      let remaining = props.state.netAmount;
      let totalBallonAmount = props.state.totalBalloonAmount;
      var dt = new Date();
      ...
      arr1.push({
        Name:
          props.state.downpaymentPercentage === "100"
            ? "One Short Payment"
            : "Down Payment Amount",
        index: "-",
        Dated: dateStr,
        ...
        Amount: DownPayment,
        Balance: DiscountAmount - DownPayment - props.state?.tokenMoney,
      });
      for (let i = 1; i <= TotalNoOfInstallments + 2; i++) {
        let remainingamount =
          (i == 12 || i == 24 || i == 36 || i == 48 || i == 60) && eachBaloonInstallmentAmount > 0
            ? remainingAmount - eachBaloonInstallmentAmount
            : remainingAmount > 0
              ? remainingAmount - eachinstallmentvalue
              : 0;
        if (
          (i == 12 || i == 24 || i == 36 || i == 48 || i == 60) &&
          remainingAmount > 0 && eachBaloonInstallmentAmount > 0
        ) {
          var new_startDate = new Date(dateStr);
          var dateConvert = moment(new_startDate).format("YYYY-MM-DD");
          ...
          if (remainingAmount >= 0 && eachBaloonInstallmentAmount > 0 && noOfBallon>0) {
            remainingAmount = remainingAmount - eachBaloonInstallmentAmount;
            noOfBallon = noOfBallon - 1;
            arr1.push({
              Name: "Balloon Installment",
              index: i,
              Dated: dateStr,
              Amount: eachBaloonInstallmentAmount?.toFixed(2),
              Balance: remainingAmount < 0 ? 0 : remainingAmount?.toFixed(2),
            });
          }
          ...
          arr1.push({
            ...
            Dated: dateStr,
            Amount:
              //eachinstallmentvalue > 0
              //? eachinstallmentvalue?.toFixed(2)
              //:
              eachinstallmentvalue,
            Balance: remainingamount < 0 ? 0 : remainingamount?.toFixed(2),
