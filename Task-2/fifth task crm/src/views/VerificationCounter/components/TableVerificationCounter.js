...
const TableVerificationCounter = (props) => {
        ...
        <tbody>
          {paginatedPosts !== null && paginatedPosts !== undefined ? (
            paginatedPosts.map((customer, index) => {
              let date = customer.created_at?.split("T")[0];
              return (
                <tr key={index}>
                  <td>{index + 1}</td>
                  <td>{customer.VerifyPersonName}</td>
                  <td>{customer.FromId}</td>
                  <td>{customer.VerifyPersonCNIC}</td>
                  <td>{date}</td>
                </tr>
              );
            })
...
export default TableVerificationCounter;
